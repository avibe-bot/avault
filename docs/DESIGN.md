...
- **Where the key lives:** the strongest implemented OS store where available — macOS Keychain today, later Secure Enclave / Linux TPM — with the file-store floor as the headless fallback (0600 + `mlock` on Unix; protected owner-only DACL + `VirtualLock` on Windows). On macOS, `auto` preserves existing `machine.key` installs by loading the file key first and mirroring it into Keychain when possible. If Keychain is unavailable, `auto` only falls back to an existing file key; it does not mint a second root that could diverge from an inaccessible Keychain item. If file and Keychain keys both exist but differ, `auto` refuses to choose silently.
- **How it decrypts:** `avault` asks the selected store to release/use the master key. The current macOS Keychain backend stores a regular generic-password item without user-presence / biometry access control, creates the item with no-clobber semantics, and hardens process crash-dump behavior before reading raw Keychain bytes. Standard-tier use stays headless after normal OS/keychain access is available. macOS may still ask once to allow a newly installed `avault` binary to access the Keychain item; that is not a per-use Touch ID / passcode policy. Future Secure Enclave / TPM backends can make the wrapping key non-extractable while still avoiding per-use human authentication.
