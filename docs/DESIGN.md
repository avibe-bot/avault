...
- **Where the key lives:** the strongest implemented OS store where available — macOS Keychain today, later Secure Enclave / Linux TPM — with the file-store floor as the headless fallback (0600 + `mlock` on Unix; protected owner-only DACL + `VirtualLock` on Windows). On macOS, `auto` preserves existing `machine.key` installs by loading the file key first, mirroring it into Keychain when possible, and falling back to the file key when Keychain is unavailable. If file and Keychain keys both exist but differ, `auto` refuses to choose silently.
...
- **Master-key store on macOS:** default `auto` uses Keychain generic-password storage for the standard-tier master key when available, but preserves existing file-store keys and keeps the file-store floor for headless sessions where Keychain is unavailable. It deliberately does not attach Touch ID / user-presence access-control flags because standard-tier secrets must be usable by unattended agents. A first-run Keychain application-access prompt may still appear for a newly installed binary.
